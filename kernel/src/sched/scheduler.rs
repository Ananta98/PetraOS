...
//! and provides:
//!
//! * `spawn_thread` — adds a thread to the least-loaded CPU's run queue.
//! * `schedule`   — triggers a scheduling decision on a specific CPU.
//! * `thread_tick`  — advances timer accounting on a specific CPU.
//!
//! # Design note
//!
//! Because PetraOS does not yet have a `proc` module or real context-switching
//! infrastructure, `schedule` merely returns the `ThreadId` chosen by the
...
/// Advance timer accounting for `cpu_id` by one tick and return the next thread
...
///
/// Returns `Some(ThreadId)` when a runnable thread is available, `None` when idle.
pub fn tick_and_schedule(cpu_id: u32) -> Option<ThreadId> {
    let mut sched = GLOBAL_SCHEDULER.lock();
    sched.thread_tick(cpu_id, TICK_NS);
...
impl GlobalScheduler {
    ...
    /// Spawn a thread onto the CPU with the fewest runnable threads.
    ///
    /// If `preferred_cpu` is `Some(id)` and that CPU is registered and idle
    /// (or its load is the minimum), the thread is placed there.
    ///
    /// Returns the CPU index the thread was assigned to, or `None` if no CPUs
    /// are registered.
    pub fn spawn_thread(&mut self, thread: SchedThread, preferred_cpu: Option<u32>) -> Option<u32> {
        if self.cpu_count == 0 {
            return None;
        }
    ...
    /// Remove a thread by `id` from all CPUs (it may only be on one).
    ///
    /// Returns the thread and the CPU it was on, if found.
    pub fn remove_thread(&mut self, id: ThreadId) -> Option<(SchedThread, u32)> {
        for (idx, slot) in self.cpus.iter_mut().enumerate() {
            let Some(cpu) = slot else { continue };
            if let Some(thread) = cpu.remove_thread(id) {
                return Some((thread, idx as u32));
            }
        }
        None
    }
    ...
    ///
    /// Returns the `ThreadId` of the selected thread, or `None` if the CPU is
    /// idle or not registered.
    pub fn schedule(&mut self, cpu_id: u32) -> Option<ThreadId> {
    ...
    pub fn thread_tick(&mut self, cpu_id: u32, delta_ns: u64) {
        if let Some(cpu) = self.cpus[cpu_id as usize].as_mut() {
            cpu.thread_tick(delta_ns);
        }
    }
    ...
    /// Directly sets the currently running thread for a specific CPU.
    pub fn set_running_thread(&mut self, cpu_id: u32, thread: SchedThread) {
        if let Some(cpu) = self.cpu_mut(cpu_id) {
            cpu.running = Some(thread);
        }
    }
}
