...
pub fn create_init_process() -> Result<(Arc<Mutex<Process>>, u64, u64), &'static str> {
    ...
    // Retrieve environment variables and boot arguments from boot command line
    let boot_cmdline = CommandLine::from_boot();
    let init_env = boot_cmdline.envp();
    let boot_args = boot_cmdline.argv();
    ...
    for candidate_path in DEFAULT_INIT_EXEC_PATHS {
        ...
        if let Ok((entry_point, stack_top)) = proc.execute(candidate_path, cmdline) {
            log::debug!(
