...
pub struct ProcessTable {
    table: Spinlock<BTreeMap<ProcessId, Arc<Spinlock<Process>>>>,
}
...
pub fn register_process(proc: Arc<Spinlock<Process>>) {
...
pub fn find_process(pid: ProcessId) -> Option<Arc<Spinlock<Process>>> {
    PROCESS_TABLE.find(pid)
}

/// Find all processes belonging to a process group.
pub fn find_processes_by_pgid(pgid: ProcessId) -> Vec<Arc<Spinlock<Process>>> {
