...
/// Thread Manager for tracking threads, state transitions, and context routing.
...
impl ThreadManager {
    ...
    pub fn init_threads(&mut self, cpu_id: u32) {
        let main_tid = ThreadId(cpu_id as u64);
        let idle_tid = ThreadId((cpu_id + 100) as u64);
        ...
        let main_task = SchedThread::new_normal(main_tid);
        sched.set_running_thread(cpu_id, main_task);
    }
    ...
    pub fn exit_current_thread_prepare(&mut self, cpu_id: u32, exit_code: i32) -> ThreadId {
    ...
    pub fn switch_to_prepare(&mut self, cpu_id: u32, next_id: ThreadId) -> Option<(*mut u64, u64)> {
    ...
    pub fn current_thread_id(&self, cpu_id: u32) -> Option<ThreadId> {
...
pub fn spawn_thread(id: ThreadId, pid: ProcessId, entry: extern "C" fn(*mut u8), arg: *mut u8) {
...
/// Returns the current thread ID executing on the specified CPU.
pub fn current_thread_id(cpu_id: u32) -> Option<ThreadId> {
