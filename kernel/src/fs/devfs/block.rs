            ...
            if dev_lock.name() == self.device_name {
                if let Some(block_dev) = dev_lock.as_block_device_mut() {
                    ...
                    while read_bytes < buf.len() {
                        ...
                        block_dev.read_block(current_block, &mut temp_buf)
            ...
            if dev_lock.name() == self.device_name {
                if let Some(block_dev) = dev_lock.as_block_device_mut() {
                    ...
                    while written_bytes < buf.len() {
                        ...
                        if chunk == block_size {
                            block_dev.write_block(current_block, &buf[written_bytes..written_bytes + chunk])
                                .map_err(|_| VfsError::NotSupported)?;
                        } else {
                            let mut temp_buf = alloc::vec![0u8; block_size];
                            block_dev.read_block(current_block, &mut temp_buf)
                                .map_err(|_| VfsError::NotSupported)?;
                            temp_buf[..chunk].copy_from_slice(&buf[written_bytes..written_bytes + chunk]);
                            block_dev.write_block(current_block, &temp_buf)
                                .map_err(|_| VfsError::NotSupported)?;
                        }
                        written_bytes += chunk;
                    }
