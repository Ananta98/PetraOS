...
use alloc::sync::Arc;
use super::inode::{Ext2Volume, Ext2Inode};
...
pub fn ext2_readdir(volume: &Arc<Ext2Volume>, inode: &Ext2Inode) -> Result<Vec<String>, VfsError> {
    ...
    while offset < size {
        ...
        let ino = u32::from_le_bytes([block_buf[offset], block_buf[offset+1], block_buf[offset+2], block_buf[offset+3]]);
...
pub fn ext2_lookup(volume: &Arc<Ext2Volume>, inode: &Ext2Inode, name: &str) -> Result<u32, VfsError> {
    let mut offset = 0;
    let size = inode.size as usize;
    let mut block_buf = alloc::vec![0u8; size];
    volume.read_inode_data(inode, 0, &mut block_buf)?;
